...
const number = prompt("Inserez un nombre");
if (number >0){
...
if (hello === "Bonjour") {
    const response = prompt("Toi, tu es poli, tu me dis bonjour, je veux bien te parler… Comment t'appelles-tu ?");
    if (response){
        const name =response;
...
const number2 = prompt("Inserez un nombre");
if (number2 >0){
    console.log(number2 + " est positif");
} else if (number2 <=0){
    console.log(number2 + " est négatif ou nul");
}
...
// fait en classe sur une page annexe
// Écrire un programme qui teste si un nombre entré par l’utilisateur est pair ou impair.
const pairOrImpair = prompt("Veuillez écrire un nombre");
if (pairOrImpair % 2 === 0){
